        ...
        while (ptr < input.length) {
            ...
            if (classMatch = line.match(/^class\s+([a-zA-Z0-9_]+)((\s*:\s*([a-zA-Z0-9_]+)){0,1})/i)) {
                ...
                if (nextBracket !== -1 && nextBracket < nextSemi) {
                    ...
                    output[classMatch[1]] = this.cfg2json(`${input.slice(innerStart, ptr - 1).trim()}\n`, level);
                    if (classMatch[4]) {
                ...
                // eslint-disable-next-line prefer-destructuring
                output[stringMatch[1]] = (_a = stringMatch[2]) !== null && _a !== void 0 ? _a : '';
                // eslint-disable-next-line no-cond-assign
            }
    ...
    json2cfg(input, indent) {
        ...
        Object.keys(input).forEach((key) => {
            ...
            switch (type) {
                ...
                case 'object': {
                    output.push(`${tabs}class ${key}`, `${tabs}{`, this.json2cfg(val, indent + 1), `${tabs}};`);
                    break;
                }
            }
        });
